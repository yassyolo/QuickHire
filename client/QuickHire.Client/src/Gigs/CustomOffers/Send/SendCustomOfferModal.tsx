...
export function SendCustomOfferModal({
    ...
    const [projectBriefForm, setProjectBriefForm] = useState<ProjectBriefForm | null>(null);
    const [showGigsChoice, setShowGigsChoice] = useState<boolean>(false);
    ...
    useEffect(() => {
        const fetchChooseFromGigs = async () => {
            try {
                const response = await axios.get<ProjectBriefForm>(`https://localhost:7267/project-briefs/form`);
                setProjectBriefForm(response.data);
            } catch (error) {
    ...
    const handleSendOffer = async () => {
        ...
        const payload = {
            ...
            revisions,
            total,
            inclusives: chosenInclusives.map((x) => x.value)
        };
    ...
    return (
        <div className="custom-offer-modal-overlay">
            <div className="custom-offer-modal d-flex flex-row justify-content-between">
                <IconButton icon={<i className="bi bi-x"></i>} onClick={onClose} className={"Close gig preview"} ariaLabel={"Close gig preview"} />
                {showGigsChoice ? (
                    <div className="choose-from-gigs">
                        <div className="custom-offer-header">Choose from Gigs</div>
                        <ul className="gigs-list">
                            {projectBriefForm?.gigs.map((gig) => (
                                <li key={gig.id} className="gig-item" onClick={() => handleGigChoice(gig)}>
                                    <img src={gig.imageUrl} alt={gig.title} className="gig-image" />
                                    <span className="gig-title">{gig.title}</span>
                                </li>
                            ))}
                        </ul>
                        <button className="btn btn-secondary" onClick={() => setShowGigsChoice(false)}>Next</button>
                    </div>
                ) : (
                    <div className="custom-offer-form">
                        ...
                        {chosenGig && (
                            <div className="gig-item">
                                <img src={chosenGig.imageUrl} alt={chosenGig.title} className="gig-image" />
                                <span className="gig-title">{chosenGig.title}</span>
                            </div>
                        )}
                        ...
                        <div className="custom-offer-inclusives">
                            ...
                            <ul className="inclusives-list">
                                {projectBriefForm?.inclusives.map((inclusive) => (
                        ...
                        <button className="btn btn-secondary" onClick={() => setShowGigsChoice(true)}>Back</button>
                        <button className="btn btn-success" onClick={handleSendOffer}>Send Offer</button>
                    </div>
                )}
            </div>
        </div>
