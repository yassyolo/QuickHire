...
interface GigModerationStatusProps {
    gigId: number;
}

export function GigModerationStatus ({ gigId }: GigModerationStatusProps) {
    const [moderationStatusDetails, setModerationStatusDetails] = useState<ModerationStatus | null>(null);
    const [showDeactivateModal, setShowDeactivateModal] = useState<boolean>(false);
    const navigate = useNavigate();

    const hanldeDeactivateSuccess = () => navigate("/admin/gigs");
    ...
    return(
        <>
            <div className="user-moderation d-flex flex-column" >
                ...
                {showDeactivateModal && <DeactivateGig id={gigId} onClose={handleDeactivateDeactivateModalVisibility} deactivateGig={hanldeDeactivateSuccess }/>}
            </div>
