...
import { isAxiosError } from "axios";import { FormGroup } from "../../../Forms/FormGroup/FormGroup";
...
export function AddSubSubCategoryModal({
  ...
  const fetchCategories = async () => {
    try {
      const response = await axios.get<Item[]>("https://localhost:7267/sub-categories/populate");
  ...
  const handleSubmit = async () => {
    try {
      ...
      const result = await axios.post("https://localhost:7267/admin/sub-sub-categories", payload);
      onAddSubSubCategorySuccess(result.data, name);
      onClose();
      resetForm();
    } catch (error: unknown) {
