...
public class SeeMoreQueryHandler : IQueryHandler<SeeMoreQuery, List<GigCardModel>>
{
    private readonly IRepository _repository;
    private readonly IUserService _userService;

    public SeeMoreQueryHandler(IRepository repository, IUserService userService)
    {
        _repository = repository;
        _userService = userService;
    }
    public async Task<List<GigCardModel>> Handle(SeeMoreQuery request, CancellationToken cancellationToken)
    {
        ...
        var browsngHistoryQueryable = _repository.GetAllIncluding<QuickHire.Domain.Users.BrowsingHistory>(x => x.Gig).Where(x => x.BuyerId == buyerId);
