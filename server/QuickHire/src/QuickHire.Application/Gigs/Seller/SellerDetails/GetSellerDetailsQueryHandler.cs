...
public class GetSellerDetailsQueryHandler : IQueryHandler<GetSellerDetailsQuery, SellerDetailsForBuyerModel>
{
    ...
    public async Task<SellerDetailsForBuyerModel> Handle(GetSellerDetailsQuery request, CancellationToken cancellationToken)
    {
        ...
        if (request.Id.HasValue)
        {
            ...
            var seller = await _repository.GetByIdAsync<Domain.Users.Seller, int>(sellerId);
            seller.Clicks++;
        ...
        var userLanguageQueryable = _repository.GetAllIncluding<Domain.Users.UserLanguage>(x => x.Language).Where(x => x.UserId == sellerUser.Id);
        var userLanguageList = await _repository.ToListAsync(userLanguageQueryable);
        ...
        var skillsQueryable = _repository.GetAllReadOnly<Domain.Users.Skill>().Where(x => x.SellerId == sellerId);
        var skillsList = await _repository.ToListAsync(skillsQueryable);
        ...
        var educationQueryable = _repository.GetAllIncluding<Domain.Users.Education>().Where(x => x.SellerId == sellerId);
        var educationList = await _repository.ToListAsync(educationQueryable);
        ...
        var certificationQueryable = _repository.GetAllReadOnly<Domain.Users.Certification>().Where(x => x.SellerId == sellerId);
        var certificationList = await _repository.ToListAsync(certificationQueryable);
        ...
        var portfolioQueryable = _repository.GetAllIncluding<Domain.Users.Portfolio>(x => x.MainCategory).Where(x => x.SellerId == sellerId);
        var portfolioList = await _repository.ToListAsync(portfolioQueryable);
        ...
        var ordersQueryable = _repository.GetAllIncluding<Domain.Orders.Order>(x => x.Reviews).Where(x => x.SellerId == sellerId);
